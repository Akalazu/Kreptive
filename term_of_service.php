<?php
include_once 'header.php';
?>

<style>
    p {
        margin: 15px 0;
        color: #1c2a46;
    }
</style>

<h1 class="d-block p-5 text-white text-center" style="background: #1c2b46; margin: 30px 0 50px 0; font-size: 35px">Terms of Service</h1>

<div class="container my-5">
    <!-- Main Section -->
    <section class="row justify-content-center">
        <div class="col-lg-10 col-md-12">
            <p class="text-justify">
                Greetings from Kreptive, a Non-Fungible Token (NFT) trading platform. The laws and regulations for using our marketplace are described in these Terms & Conditions. You accept these terms by using Kreptive and accessing it. Before engaging in any transactions on our marketplace, kindly read these carefully.
            </p>
        </div>

        <!-- General Terms Section -->
        <div class="col-lg-10 col-md-12">
            <h3 class="mt-5 mb-3">1. General Terms</h3>
            <div class="ms-3">
                <h5>1.1 Eligibility</h5>
                <p>Users must be 18 years or older, or legally able to enter binding agreements. By accessing Kreptive, you affirm that you fulfill these requirements.</p>
                <h5>1.2 Account Setup</h5>
                <p>To buy or sell NFTs on Kreptive, you need to set up an account with accurate, current information. You are responsible for keeping your account information secure.</p>
                <h5>1.3 Charges and Payments</h5>
                <p>Each NFT listing incurs a minting fee of 0.1 ETH. Kreptive applies a 10% commission on every sale, which is to be paid in ETH.</p>
                <h5>1.4 Legal Compliance</h5>
                <p>By using Kreptive, you agree to follow all applicable laws and confirm that your activities on the platform are lawful in your area.</p>
                <h5>1.5 Platform Role</h5>
                <p>Kreptive functions as a facilitator and neither owns nor endorses any NFTs displayed on the site. All transactions and agreements are the responsibility of the participating users.</p>
            </div>
        </div>

        <!-- NFT Listings Section -->
        <div class="col-lg-10 col-md-12">
            <h3 class="mt-5 mb-3">2. NFT Listings and Transactions</h3>
            <div class="ms-3">
                <h5>2.1 Ownership Verification</h5>
                <p>Sellers must possess legitimate ownership of any NFTs they put up for sale. Kreptive may request proof to confirm these ownership rights.</p>
                <h5>2.2 Restricted Content</h5>
                <p>Kreptive does not allow content that is illegal, deceptive, offensive, or violates intellectual property. Breaches may result in account suspension or closure.</p>
                <h5>2.3 Conflict Resolution and Refunds</h5>
                <p>Users are encouraged to handle disagreements directly. Kreptive does not mediate disputes or process refunds or returns unless legally obligated.</p>
            </div>
        </div>

        <!-- Intellectual Property Section -->
        <div class="col-lg-10 col-md-12">
            <h3 class="mt-5 mb-3">3. Intellectual Property</h3>
            <div class="ms-3">
                <h5>3.1 Intellectual Property</h5>
                <p>Respect for intellectual property is essential. Users are responsible for securing permission to use copyrighted elements in their NFTs.</p>
                <h5>3.2 Usage Rights</h5>
                <p>When listing NFTs, you grant Kreptive a non-exclusive, royalty-free right to display and promote your NFTs on the platform.</p>
            </div>
        </div>

        <!-- Termination and Suspension Section -->
        <div class="col-lg-10 col-md-12">
            <h3 class="mt-5 mb-3">4. Account Suspension and Termination</h3>
            <div class="ms-3">
                <h5>4.1 Account Termination</h5>
                <p>Kreptive reserves the authority to suspend or terminate accounts due to rule violations or potential fraudulent behavior.</p>
                <h5>4.2 Permanent Account Removal</h5>
                <p>To delete your account, please reach out to support. Account deletion is final, though Kreptive may retain some data to meet legal requirements.</p>
            </div>
        </div>

        <!-- Limitation of Liability Section -->
        <div class="col-lg-10 col-md-12">
            <h3 class="mt-5 mb-3">5. Limitation of Liability</h3>
            <div class="ms-3">
                <h5>5.1 Limitations on Liability</h5>
                <p>Kreptive is not responsible for any direct, indirect, incidental, or punitive damages resulting from platform usage, including data loss or harm to reputation.</p>
                <h5>5.2 Security Assurance</h5>
                <p>We make efforts to ensure platform security but cannot fully protect against unauthorized access. Use Kreptive at your own discretion.</p>
                <h5>5.3 User Disagreements</h5>
                <p>Kreptive does not mediate disputes among users. Users should resolve disagreements with each other directly.</p>
                <h5>5.4 Regional Exclusions</h5>
                <p>Some regions may not allow the limitation of certain liabilities. The limitations stated here may not apply in those areas.</p>
            </div>
        </div>

        <!-- Amendments Section -->
        <div class="col-lg-10 col-md-12">
            <h3 class="mt-5 mb-3">6. Amendments</h3>
            <div class="ms-3">
                <h5>6.1 Amendment Rights</h5>
                <p>Kreptive may change these terms without prior notice. Please review them periodically for any updates.</p>
                <h5>6.2 Acceptance of Updates</h5>
                <p>Continuing to use the platform after any changes implies your acceptance of the revised terms. Stop using the platform if you disagree with any updates.</p>
                <h5>6.3 Supplementary Terms</h5>
                <p>Certain features or services may have specific terms that override these general terms in cases of conflict.</p>
            </div>
        </div>

        <!-- Amendments Section -->
        <div class="col-lg-10 col-md-12">
            <h3 class="mt-5 mb-3">7. Community</h3>
            <div class="ms-3">
                <h5>7.1 Community Access</h5>
                <p>The community is an exclusive privilege reserved for verified users. It provides a platform where members can interact, share ideas, and connect with like-minded individuals.</p>

                <h5>7.2 Community Guidelines</h5>
                <p>To maintain a safe and welcoming environment, all members are expected to follow our community guidelines. These guidelines include respecting others, avoiding spam, and fostering positive interactions. Violation of these rules may result in restricted access or removal from the community.</p>

                <h5>7.3 Benefits of the Community</h5>
                <p>Being part of the community allows verified users to network, collaborate on projects, exchange valuable insights, and participate in exclusive events and discussions designed to enhance their experience on the platform.</p>

                <h5>7.4 Updates to Community Policies</h5>
                <p>We may update the community policies as needed to ensure a safe and effective space for all members. By continuing to participate in the community, you agree to comply with any updated policies. If you disagree with any changes, you may opt to leave the community at any time.</p>
            </div>
        </div>

    </section>
</div>

<?php
include_once 'footer.php';
?>